...
import { signInWithEmailAndPassword, sendEmailVerification, signOut, signInWithPopup, GoogleAuthProvider } from 'firebase/auth';
...
const Login: React.FC<{ setAuth: (a: AuthState) => void }> = ({ setAuth }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userRole = email === 'info.devbady@gmail.com' ? UserRole.ADMIN : UserRole.USER;
    ...
    return (
      <div className="min-h-screen bg-[#000000] flex items-center justify-center px-4 py-20">
        <div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 p-12 shadow-2xl relative text-center">
          ...
          <button onClick={() => setVerificationRequired(false)} className="block w-full py-4 border border-white/10 text-white font-black uppercase text-[10px] tracking-widest hover:bg-white/5 transition-all">
            Login
          </button>
        </div>
      </div>
